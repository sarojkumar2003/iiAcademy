...
/**
 * FrontendInquiryPage.jsx
 *
 * - Same UX & behavior as the Backend inquiry page
 * - Uses VITE_API_BASE_URL fallback to http://localhost:5000
 * - Reads course data from navigation state: location.state.course (optional)
...
export default function FrontendInquiryPage() {
  ...
  const [form, setForm] = useState({
    ...
    email: "",
    phone: "",
    course: passedCourse?.title || "Frontend Development",
    courseId: passedCourse?.id || "",
    amount: passedCourse?.amount || "",
    startDate: "",
    education: "",
    city: "",
    message: "",
  ...
  useEffect(() => {
    if (passedCourse) {
      setForm((f) => ({
        ...f,
        course: passedCourse.title || f.course,
        courseId: passedCourse.id || f.courseId,
        amount: passedCourse.amount || f.amount,
      }));
    }
        ...
        <div className="mb-8 bg-white rounded-2xl shadow-lg p-8 flex flex-col md:flex-row gap-8">
          <div className="flex-1">
            ...
            <h1 className="mt-2 text-3xl md:text-4xl font-bold text-slate-900">
              {form.course || "Frontend Development"}
            </h1>
            <p className="mt-3 text-slate-600 text-sm md:text-base">
              Build beautiful, responsive user interfaces with HTML, CSS, JavaScript and React — real projects, live sessions and portfolio-ready work.
            </p>
            ...
            <div className="mt-6">
              ...
              <ul className="mt-2 list-disc list-inside text-sm text-slate-700 space-y-1">
                ...
                <li>React fundamentals, hooks & state management</li>
                <li>Projects, deployment & portfolio building</li>
              </ul>
            </div>
          </div>
          ...
          <div className="w-full md:w-72 bg-gradient-to-b from-blue-600 to-indigo-600 text-white rounded-2xl p-6 flex flex-col justify-between">
            <div>
              <h3 className="text-lg font-semibold tracking-tight">Why this course?</h3>
              <p className="mt-2 text-sm text-blue-50">Perfect for beginners who want to become frontend or UI developers quickly.</p>
              <div className="mt-4 space-y-2 text-sm">
                <p>✅ UI/UX focused projects</p>
                <p>✅ Live code reviews</p>
                <p>✅ Portfolio & interview prep</p>
              </div>

              {/* Show selected course amount if available */}
              {form.amount ? (
                <div className="mt-4 bg-white/10 p-3 rounded">
                  <p className="text-xs">Selected</p>
                  <p className="font-semibold text-lg">{form.course}</p>
                  <p className="mt-1 text-sm">Fee: ₹{form.amount}</p>
                </div>
              ) : null}
            </div>
            ...
            <div className="md:w-1/3 bg-gradient-to-b from-blue-600 to-indigo-600 p-8 text-white">
              ...
              <div className="mt-6 space-y-4">
                <div className="bg-white/10 p-4 rounded-lg">
                  ...
                  <p className="mt-1 text-lg font-semibold">{form.course || "Frontend Development"}</p>
                  <p className="text-sm opacity-90 mt-1">HTML • CSS • JavaScript • React</p>
                </div>

                <div className="bg-white/10 p-4 rounded-lg">
                  <h4 className="text-sm font-medium">Duration</h4>
                  <p className="mt-1 text-lg font-semibold">1 Months</p>
                  <p className="text-sm opacity-90 mt-1">Live Projects</p>
                </div>
